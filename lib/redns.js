...
function FallbackEntry(hostname, options, ttl) {
  this.hostname = hostname;
  this.options = options;
  this.expiresAt = Date.now() + ttl * 1000;
}

FallbackEntry.prototype.use = function use(all, callback) {
  const now = Date.now();

  if (now >= this.expiresAt) {
    debug('fallback expired');
    return false;
  }

  dnsLookup(this.hostname, this.options, callback);
...
ReDNS.prototype.lookup = function lookup(hostname, options, callback) {
  ...
  // Create a shallow copy of user input
  ...
  const fail = (err) => {
    debug('creating fallback entry err=%s', err.message);
    const entry = new FallbackEntry(hostname, options, this.options.maxTTL);
    onEntry(entry);
  };
...
ReDNS.prototype._resolveBoth = function _resolveBoth(hostname, all, callback) {
  ...
  async.parallel({
    ...
    if (results.a.length === 0 && results.aaaa.length === 0) {
      const err = new Error('No DNS query results');
      err.code = 'ENODATA';
      return callback(err);
    }
